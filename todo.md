...
| File | Codec | Status |
|------|-------|--------|
| [todo-h264.md](todo-h264.md) | H.264/AVC decoder | 2026-08-26 sessions #32f-#32l (MBAFF deblock default-on, bit-exact I frames, P near-exact; see todo-h264.md): **CAVLC MBAFF I-slice FIXED — first pixel-exact MBAFF frame.** `parse_i_slice` used plain raster addressing while MBAFF addresses enumerate macroblock PAIRS; every bottom MB read neighbour contexts from wrong grid slots and the parse died at MB6 ("non-intra mb_type in I-slice"). Fixed with pair-based addressing + own frame-MB grid commit (CAVLC twin of #32e's CABAC grid_idx bug); mbaff_cavlc_ip I frame now SAD=0 vs ffmpeg; P-slice CAVLC MBAFF parse also implemented (ffmpeg-exact skip-run/field-flag flow) — P pixels still off pending MBAFF field MC (FIX_MV_MBAFF now implemented in mv.rs; next: parity-aware recon). 231 lib tests green, full --tests suite green. Remaining: CABAC MBAFF residual desync (#32e item 6 — decisive real-C replay harness), P/B-slice MBAFF parse, MVP row-doubling/halving, field deblocking flags, G.5 interlaced recon, H pixel_exact flip (needs ITU vectors + broader corpus); earlier #32/#32b root cause: `amvd_sum` used the spec 8.4.1.2 bottom-row/top-right sample rule while ffmpeg/x264 use DECODE_CABAC_MB_MVD's literal scan8-adjacent cells (top-left block's left/top neighbours); they disagree after 16x8/8x16/P_8x8 neighbours with per-region differing mvds, drifting the engine and misclassifying row-2 MBs as intra-in-P. Fixed in ctx.rs (amvd_sum + ref_idx_gt0_neighbors). Engine EXONERATED via new dbg_engine_diff.rs (mechanical port of real ffmpeg cabac arithmetic + runtime-parsed tables, full lockstep); MC exonerated (qpel SAD brute force SAD=0 at parsed MVs); visit-order ambiguity resolved (ffmpeg index passes through scan8 = our raster_of_8x8_sub). RESULT: dbg_qpel_brute variant_matrix ALL 10 configs BIT-EXACT vs ffmpeg pixels incl. base bframes=1+p8x8; all conformance suites + 231 lib tests green; oracle updated to correct convention with pixel-pinned tail MBs. Remaining: G (MBAFF P/B), G.5 interlaced, H pixel_exact flip (needs ITU vectors + broader corpus) |
...
| [todo-aac.md](todo-aac.md) | Native AAC-LC decoder | Phases 1-7 COMPLETE (2026-08-23): conformance vs ffmpeg passes a real assertion (max-abs-diff 0.021 < 0.05 tolerance, channel-0 correlation 0.995); root cause of the long-standing "amplitude" gap was a Princen-Bradley/TDAC violation in `window.rs` (half-windows built with denominator `n` instead of the full length `2n`), not a scale constant. 2026-08-25: `prev_shape` fix landed (production decode path wasn't updating window shape after synthesis). 2026-08-27: baselines verified — tonal cases bit-exact (max_diff < 0.001), remaining gaps are (1) `noise_mono_44100` PNS correlation ~0.52 in EIGHT_SHORT frames, (2) `noise_stereo_44100` max_diff ~0.059 (same PNS root cause, milder), (3) `sweep_stereo_44100` single-sample outlier at frame 25 sample 488 (localized to TNS→non-TNS transition, corr 1.0000). All correctly excluded from aggregate gate. All 75 AAC tests + clippy + fmt clean. |
