...
| [todo-av1.md](todo-av1.md) | AV1 decoder | 2026-08-23: fixed a broken build and two real bugs in the Phase G.0 capture bridge (wrong pre/post snapshot timing; arithmetic-decoder resume state re-derived instead of captured). Bridge now proves `coeffs()` correct on two previously-suspect blocks. Hand-verified dequant/inverse-transform/filter-intra-prediction are ALSO correct for testsrc's first block; caught+reverted a wrong `Mrow`/`Mcol` scan "fix" via empirical PSNR regression. Then discovered this environment has working internet access and live-fetched the AV1 spec's own tables/pseudocode, byte-diffing every table/control-flow decision in that block's entire mode-parsing sequence (partition/skip/y_mode/uv_mode/filter_intra/intra_tx_type) — all matched exactly. Found one real but currently-inert gap (`read_cdef`/`read_delta_qindex`/`read_delta_lf` never implemented) that doesn't affect this corpus. Root cause of the actual pixel divergence still not found despite this being the deepest verification pass yet — solid-color content still pixel-exact (99dB); real/textured content still noise-level (~10-17dB) |
