...
| [todo-av1.md](todo-av1.md) | AV1 decoder | 2026-09-03 (see todo-av1.md): intra keyframe reconstruction essentially solved; loop filter (deblock+CDEF) bugs fixed and now genuinely improve PSNR (`testsrc` Y 61.95→73.01); loop restoration (§7.17) implemented but apply gated off pending a restoration-unit-boundary fix; IBC reconstruction exists but only for the simple case — a dav1d-trace-diff session fixed a real bug (skipped transform blocks weren't resetting their coefficient neighbour context, via new `coeff::clear_coeff_context`) taking testsrc2 Y/U/V 14.36/17.17/13.92→**23.11/25.08/16.95**, then hit and confirmed the next real IBC gap: blocks needing the var-tx tree + an inter `tx_type` + inter coefficient context, which `reconstruct_ibc_block` doesn't implement (genuine Phase-E-shaped work). **Corpus (`av1_psnr_check` Y/U/V dB): solid_red 99/99/99; testsrc 73.01/53.76/49.23; mandelbrot 47.59/52.44/52.62; smptebars 57.49/99/99; testsrc2 23.11/25.08/16.95.** A follow-up 2026-09-03 session redirected the long-standing "mandelbrot directional-prediction residual" item: a real pre-filter dav1d reference (not the confounded post-filter one prior sessions compared against) shows mandelbrot's prediction+transform are very likely already bit-exact for the mechanisms previously suspected (sparse-coefficient DC, SMOOTH_H sub-block borders, rectangular sqrt(2) rescale — three representative blocks all confirmed exact); the remaining ~47dB gap is concentrated in an 18-row band (rows 71-88 of 96) consistent with a loop-filter difference, not reconstruction — traced one step further (same session) to a likely root cause: `deblock_plane` has no transform/prediction-edge *presence* check, so it filters at every 8px grid line including ones strictly inside a single wide (>8px) transform, e.g. `x=8` inside a 16-wide transform where the only real edges are at `x=0`/`x=16`; CDEF's math was verified formula-correct and just propagates the resulting ±1-2px deblock error forward. **Fix implemented and verified the same session**: `FrameMeta` gained `luma_edge_left/top` + `chroma_edge_left/top` grids and `mark_luma_edges`/`mark_chroma_edges`, called per real transform sub-block from `reconstruct/intra_block.rs`'s luma/chroma loops (keyframe + IBC paths); `deblock_plane` now skips any grid line neither grid marks as a real edge. `av1_psnr_check`: mandelbrot Y/U/V 47.59/52.44/52.62→**47.62/52.53/52.68**, testsrc U 53.76→**53.93**, rest unchanged — smaller than initially hypothesized (most content already uses ≤8px transforms where this was a no-op) but a real, verified spec-correctness fix with no regressions. `inter_block.rs` (true inter, unreached by this keyframe-only corpus) still needs the same treatment when Phase E lands. Remaining, in priority order: (1) further loop-filter verification — re-run the same trace method now that this bug class is gone; (2) loop restoration boundary-pixel fix to un-gate apply; (3) IBC var-tx tree + inter tx_type + inter coeff context — blocks testsrc2; (4) inter Phase E — `Ok(None)` for non-keyframes (also needs the edge-marking above); (5) then flip `capabilities().pixel_exact`. 128 unit tests pass, clippy clean. |
