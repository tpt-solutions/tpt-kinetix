...
> Last reconciled: 2026-09-05 (index rows refreshed 2026-09-26 — H.264 row and
> the VP9 roadmap bullet updated against todo-h264.md/todo-vp9.md; AV1/AAC rows
> below and the long correction/session-notes preamble were not re-verified
> this pass, see todo-av1.md/todo-aac.md for their current state instead).
> Monolithic todo.md split into per-codec files.
...
- ~~**AAC decoder work.**~~ **Moved out of this repo (2026-09-16, reconfirmed
  2026-09-26): AAC now lives in the separate `tpt-cadence` audio-codec repo**
  (`D:\Programming\1PRODUCTION\Open Source\tpt-cadence`). `tpt-kinetix-aac` was
  deleted; todo-aac.md's history below this point (the 2026-09-10 ISO
  conformance work: 17 synthetic cases + 3 ISO vectors bit-exact, remaining
...
- **Royalty-free codec expansion — VP9 done; audio codecs moved out.**
  ...
  the pipeline/CLI, fuzzed in CI. **Opus and MP3 decode are no longer this
...
| File | Codec | Status |
|------|-------|--------|
| [todo-h264.md](todo-h264.md) | H.264/AVC decoder | **2026-09-26, session #32ce (see todo-h264.md for full per-session history through #32af-#32ce):** progressive + PAFF + MBAFF are all bit-exact vs ffmpeg/JM (`capabilities().pixel_exact == true`); this arc has been closing the last mixed-frame/field PAFF B-slice temporal-direct-mode gaps (§8.4.1.2.3) against a local JM (`ldecod`) oracle. #32ce transcribed JM's field/frame co-located-picture conversion rules (`ColPairCtx`, `rsd` outer-corner mapping, frame↔field vertical-unit conversion) into `mv.rs`/`ref_pic.rs`; `CAPA1_TOSHIBA_B`/`CVPA1_TOSHIBA_B` went from a 4-frame strict abort to full-stream decode with several previously-garbage frames now landing in position. **Remaining (next session), in priority order:** (1) field B pairs whose co-located picture is another field pair (`Sharp_MP_PAFF_1r2` frames 5/11/13`, CAPA1/CVPA1 display-7 class — same-kind temporal-direct path, 10-33k wrong samples despite bit-identical parse/pre-deblock planes vs JM); (2) frame-coded single B pictures (Case A path, e.g. CAPA1 display 2/7) need a per-MB MV/pixel diff vs JM; (3) CAPA1 aborts at NAL 143 (next unsupported slice) plus small (≤38-sample, max ≤2) near-exact residue on frames 3/4/6/16 — likely deblock-rounding, triage after (1)+(2); (4) `HCHP1_HHI_B` (intra-4×4 DC neighbour availability) and `CAMA1_Sony_C` (CABAC MBAFF-I desync) still open from earlier sessions. Full multi-slice-picture decode (slices 2+) and full-resync recovery are still not implemented. Gates as of #32ce: 273 lib tests, 72 integration binaries, ITU suite 33/33 hard-checked BitExact, fmt/clippy `-D warnings` clean. |
...
- [x] ~~**Opus decode (after VP9)**~~ — **out of scope for this repo (2026-09-26):
      moved to the separate `tpt-cadence` audio-codec repo**
      (`D:\Programming\1PRODUCTION\Open Source\tpt-cadence`), alongside AAC.
      Don't add a `tpt-kinetix-opus` crate here; audio decoders belong in
      tpt-cadence going forward.
- [x] ~~**MP3 decode (filler)**~~ — **out of scope for this repo (2026-09-26):
      moved to the separate `tpt-cadence` audio-codec repo**
      (`D:\Programming\1PRODUCTION\Open Source\tpt-cadence`), alongside AAC
