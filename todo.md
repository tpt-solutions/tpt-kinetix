...
> Last reconciled: 2026-09-05. Monolithic todo.md split into per-codec files.
> For the long session-notes preamble and infrastructure phases 0–11, scroll past this index.

> **CORRECTION (same day, later session): the "12/12 BitExact clips still
> exact" claim two lines below is wrong, and so is this table's matching
> claim below it.** `cargo test -p tpt-kinetix-h264 --test itu_conformance`
> was run without `--nocapture`, which hides a passing test's stdout —
> so the run actually printed (and hid) "no fixtures under .../tests/
> fixtures/itu — run `just fetch-h264-conformance`. Skipping." the whole
> time. This container has **no ITU conformance fixtures on disk and no
> working path to fetch them**: `tools/fetch-h264-conformance.sh` downloads
> from `itu.int`, which returns HTTP 403 through this environment's proxy.
> So neither this reconciliation nor the 2026-09-05 H.264 sessions below that
> cite "full ITU conformance suite passes" actually exercised any real ITU
> bitstream in *this* container — that claim is carried from whatever
> environment those commits' own session notes were written in (which
> evidently did have the fixtures), not re-verified here. Always pass
> `--nocapture` (or check for the skip line) when running this test.
> Everything else this note verified (`cargo test --lib`, clippy, fmt) does
> not depend on external fixtures and stands.
>
...
> scratch code the session notes below repeatedly flag, except this batch had
> already been committed with the lint violations live. All four are
> mechanical fixes (`!(x != 0)` → `x == 0`, drop a same-type cast), no
> behavior change; `cargo clippy --workspace --all-targets -- -D warnings` is
> clean again. **Could not independently re-verify AV1/H.264 PSNR or dav1d
> bit-exactness figures cited in the session notes below** — this environment
> has no `ffmpeg`/`dav1d` on `PATH`, so every `ffmpeg`/`dav1d`-gated
> conformance test (`av1_psnr_check`, `av1_*_vs_dav1d_when_available`) skips
> here rather than running; those numbers are taken on trust from the commits'
> own session notes, not re-measured this session. **One real prose
> discrepancy found**: the AV1 priority list below (item 5, carried from
> 2026-09-04) says inter prediction is "unimplemented" and the decoder
> "returns `Ok(None)` for non-keyframes" — this is stale. `decode_block`
> (`reconstruct/partition.rs:486`) has dispatched to a real
> `decode_inter_block` (`reconstruct/inter_block.rs`) since 2026-08-16 (MV
> candidate list, `read_mv`, single/compound reference decode, 8-tap sub-pel
> MC), and `Av1Decoder::capabilities()` currently claims this inter path
> "runs end-to-end on multi-frame streams — validated frame-by-frame vs
> dav1d." Since `dav1d` isn't available to re-run that validation here, this
> reconciliation can't confirm which claim is more accurate — flagging both
> instead of silently picking one; next session should re-run
> `av1_inter_corpus_vs_dav1d_when_available` on a machine with `dav1d`/
> `ffmpeg` and correct whichever side is wrong (either soften
> `capabilities()`'s notes string, or drop "unimplemented" from the priority
> item below).

### Non-H.264 open work (2026-09-01 reconciliation, AV1/H.264 sections updated 2026-09-05)
...
- **AV1 decoder — the largest remaining item.** 2026-09-04: intra keyframe
  ...
  **Remaining AV1 open items, in priority order:** (1) find the pixel-
  ...
  inter prediction end-to-end bit-exactness is unvalidated in this
  environment (see the 2026-09-05 reconciliation note above — the
  MV-pred/inter-recon *code path* is wired and dispatched, contrary to this
  item's earlier wording; what's actually unverified is whether it's
  bit-exact, which needs a `dav1d`-equipped machine to re-check); (6)
  then flip `capabilities().pixel_exact`. **2026-09-05 same-day follow-up**:
  wired the previously-hardcoded-to-0 per-block `delta_lf` (§7.12.1) into
  `compute_level`'s loop-filter level computation (no corpus PSNR change —
  the current corpus never sets `delta_lf_present` — but closes a real spec
  gap); while doing so found and fixed a separate, larger gap:
  `inter_block.rs` never called any of `record_luma`/`record_chroma`/
  `mark_luma_edges`/`mark_chroma_edges`/`record_delta_lf`, so every
  inter-coded block left its loop-filter metadata at defaults and deblock
  was effectively blind to every P/B-frame edge — fixed by only gating the
  residual-read calls (not the geometry-recording calls) on `skip`. Also
  spent a session chasing a `mandelbrot` deblock/CDEF divergence at
  `px=(62,1)`: independently re-verified the wide-filter tap formula and
  `flat`/`flat2`/`hev` mask logic line-for-line against dav1d source (both
  correct), but found the `KINETIX_AV1_NODEBLOCK`/`NOCDEF` isolation
  technique itself has a pipeline-ordering confound (disabling deblock feeds
  CDEF un-deblocked input, so "NODEBLOCK matches dav1d" doesn't cleanly
  prove deblock is the bug) — deblock remains the prime suspect via the
  `NOCDEF`-only evidence but isn't conclusively implicated; closing this
...
| File | Codec | Status |
|------|-------|--------|
| [todo-h264.md](todo-h264.md) | H.264/AVC decoder | 2026-08-28 sessions #32p-#32r (see todo-h264.md): **CABAC MBAFF I-slice FIXED & BIT-EXACT** (#32p — a spurious `end_of_slice_flag` was decoded after every MBAFF pair-top MB, §7.3.4; `g6_cabac_i` now SAD=0 vs `-skip_loop_filter` ffmpeg ref). Progressive CABAC P/B conformance RESOLVED earlier (#32o). #32q fixed the CABAC P/B **pair-scan addressing bug** (parsers iterated plain raster while neighbour lookups used the frame-MB grid — the P/B twin of #32e/#32f) plus an MV-predictor decode-order bug in `mv.rs` (`predict_slice_mvs_ex`): `mbaff_ip` P SAD 83k→~30k, `mbaff_ibp` P 107k→46k / B 128k→75k, `mbaff_cavlc_ip2` P 5748→1154; progressive byte-identical. #32r gated `cabac_b.rs` debug prints behind `KINETIX_BINTRACE`. 246 lib tests green, clippy `-D warnings` clean. **2026-08-29→31 (#32af + PAFF field-P): `mbaff_ibp` P AND B frames now BIT-EXACT** (BUG 3 `get_dct8x8_allowed`; B_SUB_MB table order; B_8x8 mvd list-order; missing 8×8-transform branch in `reconstruct_b_inter_luma`). **PAFF field chroma BIT-EXACT** (`paff_b_field.264` chroma 190→0). Remaining H.264 gaps: (1) PAFF field **luma max_diff 68** confined to `P_8x8` MBs with a coded 8×8 group whose `sub_mb_type` is finer than 8×8 (4×4/8×4) — parse in sync, MVs+qp+scan verified, a partial CABAC 4×4-residual-coeff error; needs a bin-level CABAC oracle. 2026-09-01: investigating via env-gated sweep hooks in `reconstruct.rs` (`KINETIX_LUMA_PARITY_OFF` opposite-parity luma-MC offset hypothesis mirroring the chroma fix, + `KINETIX_PAFF_RESIDUAL_DUMP` MB(4,0) residual dump) — uncommitted, not yet a fix; (2) G.5a pin every bit-exact MBAFF frame as a hard assert; (3) G.5b add real PAFF + MBAFF corpus clips; (4) G.5c non-16 crop clip assert; (5) Phase H `pixel_exact` flip + README (needs ITU vectors + broader corpus). **2026-09-03 (#32ai, branch `h264/progressive-8x8-strict-mode`, commit `824b144`): progressive High-profile 8×8 transform now honoured in strict mode** — the blanket `transform_8x8_mode_flag` reject is gone; strict mode runs the real path and rejects only genuine scaffold fallbacks (`scaffold_fallback` flag) — multi-slice / non-4:2:0 / >8-bit. Conformance eprintln "[GAP]" checks hardened to `assert_eq!(max_diff, 0)` across `high_profile_8x8[_cabac]` / `cabac[_pframe]` P/B / `high_profile`. Stale `h264_real_sample_harness_across_profiles` (asserted pre-flip scaffold state, was failing on master) rewritten to assert bit-exact. 373 h264 tests + h264 clippy + test-utils conformance green. **2026-09-05 (#32al-#32ao, ITU-T H.264.1 conformance suite): real SPS/PPS-by-id selection bug fixed** — two call sites resolved the active SPS/PPS incorrectly for multi-PPS streams (`MPS_MW_A` improved); **`MIDR_MW_D` root-caused but not fixed**: the stream has a genuine (flag-forbidden but real, ffmpeg decodes it bit-exact vs the ITU reference) `frame_num` gap that §8.2.5.2's non-existing-picture synthesis would normally fill and nothing in the decoder implements — the gap's onset is confirmed as the cause of the divergence, but the exact pixel-level mechanism (MV-prediction context or POC math reacting to the single-real-entry DPB) isn't pinned down yet; ref-list-padding and pixel-content hypotheses are both ruled out with data (chroma near-untouched, `ref_idx=0`-only MBs equally wrong). **Real fix landed (#32ao): CABAC `end_of_slice_flag` mid-picture is now a legitimate stop, not a forced desync error** — every CABAC slice parser previously only accepted the terminate bin on the picture's true last MB, so any multi-slice picture's first slice (the only slice ever fed to the parser; continuation slices are still dropped) always "desynced" and the whole picture fell back to flat-grey scaffold; fixed via a new `ParsedSlice::decoded_mb_count` so `decoder/mod.rs` can still correctly flag `scaffold_fallback`/`NotPixelExact` for the genuinely-undecoded remaining slices while keeping the real, correctly-decoded first-slice macroblocks (verified via diffmap: `CABAST3_Sony_E` frame 1's first ~4/18 MB rows are now bit-exact where the whole frame was wrong before). Full multi-slice support (decoding slices 2+) is still not implemented. 264 lib tests pass; the "full ITU conformance suite passes, 0 failures" claim here could not actually be re-verified in this container — see the reconciliation correction above, this environment has no ITU fixtures and no working fetch path (itu.int returns 403 through the proxy). clippy/fmt clean workspace-wide (see the 2026-09-05 reconciliation note above for a `just clippy` regression found and fixed in unrelated debug-scratch test files). **2026-09-05, same day, later session: temporal direct mode (§8.4.1.2.3) implemented** — previously `direct_spatial_mv_pred_flag == 0` unconditionally bailed to the scaffold for any B slice that actually coded a Direct-type macroblock (the blocker for `CABA3_Sony_C`/`CANL3_Sony_C`/`CVBS3_Sony_C`/`CACQP3_Sony_D` per the manifest). Implemented `derive_temporal_direct`/`apply_temporal_direct` in `mv.rs`, cross-checked against FFmpeg's real `pred_temp_direct_motion` (`libavcodec/h264_direct.c`, fetched verbatim via WebFetch rather than recalled from memory): per-quadrant derivation from the co-located block's own List0-preferred-over-List1 reference/MV, `MapColToList0` via POC matching, and the `tb`/`td`/`dist_scale_factor` scaling formula. Needed new POC bookkeeping: `DpbEntry` gained `list0_poc`/`list1_poc` (that picture's own ref-list POCs, snapshotted when *it* was decoded, needed later so a colocated block's `refIdxCol` can be mapped to a physical picture); threaded a new `TemporalDirectCtx` through `predict_b_slice_mvs`/`predict_inter_b_macroblock` and both `parse_b_slice`/`parse_b_slice_cabac` (CAVLC/CABAC), wired from `decoder/mod.rs`'s progressive B-slice path where `current_poc`/`ref_l0`/`ref_l1` were already in scope. MBAFF's B-slice path still passes `None` (unimplemented there, same pre-existing fallback as before — needs field/frame-pair-aware POC bookkeeping this change didn't add). Verified: hand-derived unit tests (a halfway-B-picture case where `tb/td = 0.5` should exactly halve the co-located MV, a List1-preferred case, an intra-colocated-block zero-motion case, and a `MapColToList0` fallback-to-zero case) all pass; full workspace `cargo build`/`cargo test --lib` (all crates green, h264 268/268)/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo fmt --all --check` all clean. **Not validated end-to-end against real bitstreams** — this container cannot reach `itu.int` (see correction above), so whether `CABA3_Sony_C` et al. now actually decode bit-exact (or even just decode without a new bug) is unconfirmed; next session with real network/fixture access should run the ITU suite and expect these 4 clips to move from `KnownGap` (or reveal a new bug to fix). |
| [todo-av1.md](todo-av1.md) | AV1 decoder | 2026-09-04 (see todo-av1.md for full history): intra keyframe reconstruction solved; deblock now gated on a real transform/prediction-edge presence check (was filtering every 8px grid line unconditionally); IBC reconstruction fixed for the simple case (`coeff::clear_coeff_context`, testsrc2 Y 14.36→23.11) but still blocked on var-tx tree + inter tx_type + inter coeff context for its harder blocks (genuine Phase-E-shaped work). **★ `dqDenom` fix (2026-09-04): `smptebars` luma is now pixel-exact (57.49→99.00 dB across all 144 rows), `mandelbrot` 47.62→53.10 dB.** `dq_denom` matched `tx_size == TX_32X32`/`TX_64X64` literally instead of the spec's square-up-averaged `tx_sz_ctx`, so six rectangular sizes (`TX_16X32`/`TX_32X16`/`TX_16X64`/`TX_64X16`/`TX_32X64`/`TX_64X32`) silently got `dqDenom=1` instead of 2 or 4, overscaling their residual 2-4x; found via a `DAV1D_ITXDUMP`-patched dav1d trace on mandelbrot's `TX_16X32` `SMOOTH_V` block. **Corpus (`av1_psnr_check` Y/U/V dB): solid_red 99/99/99; testsrc 73.01/53.93/49.23; mandelbrot 53.10/52.53/52.68; smptebars 99.00/99.00/99.00; testsrc2 23.11/25.08/16.95.** Re-traced mandelbrot after the fix: reconstruction (prediction+transform) is bit-exact everywhere checked; the remaining gap (±1-9 px, scattered across many small `TX_4X4` blocks) is confirmed to live in the loop filter (deblock and/or CDEF) specifically, not reconstruction — no further systematic bug found this round despite inspection, needs the same per-edge trace method next. **Deblock luma 4-vs-8-sample-granularity fix (2026-09-04): `mandelbrot` 53.10→57.62 dB, `testsrc` 73.01→73.46 dB.** Traced the `bx=13,by=14` edge above to `deblock_plane`'s luma pass using an 8-sample grid (`edge = bx*8`) that structurally cannot represent a real transform edge at a 4-but-not-8-aligned position (e.g. `x=52`, between an independent `TX_4X8` and `TX_4X4` block) — added a parallel 4×4-luma-cell-resolution grid (`FrameMeta::w4/h4/luma_tx_w4/luma_tx_h4/luma_edge_left4/luma_edge_top4`) populated at the same per-transform-sub-block call sites, switched luma's `deblock_plane` call to `step=4` over it; chroma untouched (its own minimum tx size already matches the existing 8-luma-px grid, confirmed by unchanged U/V PSNR). **Loop-filter level derivation fix (2026-09-04 cont'd): `mandelbrot` 57.62→58.79 dB, `testsrc` 73.46→74.88 dB.** Re-traced mandelbrot's new worst pixel (row64/col96, diff=10) against dav1d with `--cpumask 0` (forces the C path so source patches actually fire — the default SIMD path silently skips them) and found dav1d's loop-filter `I=19` vs Kinetix's `18`: `LoopFilterDeltas`'s `#[derive(Default)]` gave an all-zero array instead of §7.14.4's real `setup_past_independence()` reset (`ref_deltas[INTRA_FRAME]` should default to `1`, not `0`), and `compute_level` was also missing the `nShift`-based ref-delta doubling at level ≥32 and wrongly added a mode-delta term that spec/dav1d never apply to intra blocks. Corpus (Y/U/V dB): solid_red 99/99/99; testsrc 74.88/54.07/49.26; mandelbrot 58.79/53.15/52.76; smptebars 99.00/99.00/99.00; testsrc2 23.11/25.08/16.95 (unaffected, IBC-gated). **Loop restoration boundary-pixel fix (2026-09-04 cont'd): real fix, but null result — still gated.** Isolated mandelbrot's remaining small (±1), widespread diffs (e.g. row4 cols79-93) to loop restoration via dav1d's `--inloopfilters norestoration` flag. Fixed `wiener_filter_plane`/`sgrproj_filter_plane`'s real bug — both extracted each restoration unit into an isolated buffer, so taps near a unit's own edge clamped there instead of reading real neighbouring pixels — now both read from a shared whole-plane snapshot. Honestly null on the one exercised test case though: `KINETIX_AV1_FILTER=1` on testsrc's V plane gives byte-identical 42.24 dB before and after (confirmed via `git stash` A/B), so restoration's deeper bug (Wiener/SgrProj math or `read_lr_unit` parsing, neither traced yet) is still unlocated and it stays gated off by default (default-corpus PSNR unaffected, confirmed). 133 unit tests pass, clippy clean. **Loop restoration: three real bugs found and fixed, now un-gated by default (2026-09-04 cont'd).** After the Wiener h/v swap fix (testsrc V 42.24→55.18 dB), found SgrProj (mandelbrot's path, `set=10`, `r0=0/r1=1`) was a complete silent no-op — `sgrproj_filter_plane` used the raw decoded `xqd[1]` as the second projection weight, but §7.17.4's real weight is the complement `(1<<7) - xqd[0] - xqd[1]` (cross-checked against dav1d's `lr_apply_tmpl.c`); for the real `xqd=[0,2]` case, raw weight 2 rounds every correction to 0, while the correct weight 126 doesn't. Fixed (one-line). **Result: mandelbrot Y 58.79→70.45 dB**, and diffing the restored plane directly against dav1d shows only 72/12288 pixels (±1) differ — essentially the same 71-pixel gap already present *before* restoration runs, i.e. restoration is now correct to the precision of its (separately tracked) input. With all three restoration bugs fixed (unit-boundary clamping, Wiener h/v swap, SgrProj weight complement) and zero corpus regressions, un-gated `apply_post_filters` to run restoration unconditionally instead of behind `KINETIX_AV1_FILTER=1` — it is now part of default decode output. 134 unit tests pass, clippy clean, full `tpt-kinetix-av1` test suite (integration/proptest/doctests) green. **IBC var-tx tree + inter tx_type implemented (2026-09-04 cont'd): two verified increments on the Phase-E-shaped testsrc2 gap.** Replaced `reconstruct_ibc_block`'s wrong single-symbol intra-style tx read with a real recursive var-tx-tree parse (`read_block_tx_size_ibc`/`read_tx_tree`, cross-checked against dav1d's `read_vartx_tree`/`read_tx_tree` branch-for-branch; the `txfm_split` CDF table was already scaffolded correctly, unused). Then found IBC forced every transform to `DCT_DCT` and never read real `inter_tx_type` bits; added `get_tx_set_inter`/`get_uv_inter_txtp`/`read_inter_transform_type` (also cross-checked against dav1d, its CDF tables likewise already scaffolded correctly). Verified against a real `testsrc2` IBC block traced against dav1d: Kinetix now decodes `tx=7 txtp=13 eob≈64` with the post-read decoder `rng` (`r=42504`) matching dav1d's trace *exactly* — bit-exact sync confirmed through this block's full luma residual read, not just its header. Traced one step further: sync breaks at the *next* read (this same block's chroma skip/all-zero symbol) — concrete next target is comparing Kinetix's own computed chroma skip-context (`all_zero_ctx`) against dav1d's `sctx=9` for this exact call, to determine whether the remaining "inter coefficient context" gap is in context derivation or elsewhere. No corpus regression on solid_red/smptebars/testsrc/mandelbrot; testsrc2 stays a wash for now (still desyncs one read later). 138 unit tests pass (4 new), clippy clean, `cargo build --workspace` green. **Follow-up same day: full-state (range+`dif>>48`, not range-only) verification confirmed the chroma skip context AND the decoded `all_skip` boolean both match dav1d exactly too** — the earlier-suspected divergence there was a false alarm from comparing against the wrong one of three identically-labelled `Post-uv-cf-blk` print sites in dav1d's source. Both increments now verified with real full-arithmetic-state rigor; where sync actually breaks (past this point) is still open for the next session, same technique. Remaining, in priority order: (1) resume the full-state trace past the chroma skip read to find where sync actually breaks — the concrete next step to unblock testsrc2; (2) fresh worst-edge search on mandelbrot/testsrc — this vein has found four real bugs in a row (dqDenom, ref-delta default/shift, Wiener h/v swap, SgrProj weight complement); (3) wire the currently-hardcoded-to-0 per-superblock `delta_lf` into `deblock_plane`'s level computation; (4) restoration's "mix" (both passes active) configuration and real stripe-boundary handling remain unverified (no corpus clip exercises either); (5) inter Phase E bit-exactness unvalidated (the code path itself is wired — see the "Non-H.264 open work" AV1 bullet above for the 2026-09-05 correction to this); (6) then flip `capabilities().pixel_exact`. **2026-09-05: `delta_lf` wired into loop-filter level computation + inter-block loop-filter metadata gap fixed (deblock was blind to every P/B-frame edge); mandelbrot `px=(62,1)` deblock/CDEF divergence investigated further, still open (isolation-technique confound found, not yet a clean proof).** See the AV1 bullet under "Non-H.264 open work" above for detail. |
