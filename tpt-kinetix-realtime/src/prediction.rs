//! Realtime prediction (lean `lib.rs` prediction design + DECISION 2).
//!
//! # Intra
//!
...
//! weighted prediction, no compound modes (this is exactly DECISION 2's
//! "unidirectional-P, single backward reference" shape that Realtime inherits).
//! Each block is predicted from its single reference frame by a quarter-pixel
//! motion vector using the H.264 §8.4.2.2 kernel: 6-tap separable for luma
//! half-samples, bilinear for chroma.
...
/// and whether the column/row is mirrored (so the flow points toward the
/// opposite corner).
fn directional_params(mode: IntraMode) -> (i32, bool, bool) {
    match mode {
        IntraMode::Horizontal => (0, false, false), // left[r]
        IntraMode::Vertical => (0, true, false),    // above[c]
        ...
        IntraMode::HorizontalUp => (26, false, true), // tan22 ≈ 0.404 ≈ 26/64
        IntraMode::HorizontalDown => (26, false, false),
        IntraMode::VerticalLeft => (151, false, false), // tan67 ≈ 2.356 ≈ 151/64
...
fn predict_directional(
    ...
    // Build extended reference arrays. Index `j` is the sample at offset `j`
    // from the block edge: `top[j] == above[j]` for `j < size`, then clamped;
    // `lft[j] == left[j]` likewise. Indices are always >= 0 for every mode
    // below, so no separate corner entry is needed (the corner is still used
    // by DC/Planar directly).
