...
fn generate(
    ...
    // `ffmpeg -loglevel debug`'s "8x8 transform intra: NN%" log line);
    // the flatter `testsrc` pattern was found to never trigger it, making
    // the "clip exercises 8x8" assertion below vacuously fail.
