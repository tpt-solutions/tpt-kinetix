...
/// Intra4x4Mode enum).
fn ref_preds_9(frame: &[u8], stride: usize, x0: usize, y0: usize) -> [[u8; 64]; 9] {
...
fn dbg_hp352_localize() {
    ...
    for (name, range) in [
        ("luma", 0..luma_n),
        ("cb", luma_n..luma_n + luma_n / 4),
        ("cr", luma_n + luma_n / 4..ours.len()),
    ] {
        let mut max_d = 0i32;
        let mut num = 0usize;
        ...
        eprintln!(
            "{name}: max_diff={max_d} differing={num}/{}",
    ...
    for mby in 0..mb_rows {
        for mbx in 0..mb_cols {
            if diff_grid[mby][mbx] > 0 && shown < 10 {
                eprintln!(
                    "first diffs include MB(x={mbx},y={mby}) max={} is8x8={}",
                    diff_grid[mby][mbx],
                    tracer.intra88.contains(&(mbx as u32, mby as u32))
                );
                shown += 1;
            }
        }
            ...
            for mbx in 5..12usize {
                for half_x in 0..2usize {
                    ...
                    line.push(if mx == 0 {
                        ...
                        std::char::from_digit(mx.min(9) as u32, 10).unwrap()
                    });
                }
                line.push(' ');
            }
    ...
    // Generic: locate the FIRST differing sample and dump a 20x20 diff
    // window around it.
    'find_first: for y in 0..h {
        for x in 0..w {
            if ours[y * w + x] != refyuv[y * w + x] {
                let x0 = x.saturating_sub(10);
                let y0 = y.saturating_sub(10);
                eprintln!(
                    "=== first diff at ({x},{y}); window x={x0}..{}, y={y0}..{} ===",
                    x0 + 20,
                    y0 + 20
                );
                for yy in y0..(y0 + 20).min(h) {
                    let mut s = String::new();
                    for xx in x0..(x0 + 20).min(w) {
                        let d = ours[yy * w + xx] as i32 - refyuv[yy * w + xx] as i32;
                        if d == 0 {
                            s.push_str("   .");
                        } else {
    ...
    // === Decisive analysis: scan ALL MBs for mode mismatches ===
    ...
    for mbx in 0u32..mb_cols as u32 {
        for mby in 0u32..mb_rows as u32 {
            for i8 in 0..4usize {
                let blk = 64 + i8 as u8;
                let Some(&pred) = tracer.preds.get(&(mbx, mby, blk)) else {
                    continue;
                };
