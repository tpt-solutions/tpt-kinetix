...
fn gen() -> Option<Vec<u8>> {
    let dir = std::env::temp_dir().join("tpt_kinetix_h264_mandelbrot_oracle");
...
#[test]
fn mandelbrot_iframe_cavlc_trace() {
    if !ffmpeg_available() {
    ...
    assert!(
        result.is_ok(),
        "I-slice CAVLC parse should complete without desync: {:?}",
        result.err()
    );

    // Print all recorded block info
    for block in tracer.blocks.lock().unwrap().iter() {
        eprintln!("{block}");
    }
}
