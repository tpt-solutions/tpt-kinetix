...
fn gen_cabac(dir: &std::path::Path, cabac: bool) -> Option<(Vec<u8>, Vec<u8>)> {
    let name = if cabac { "ip_cabac.h264" } else { "ip_cavlc.h264" };
    let refname = if cabac { "ip_cabac.yuv" } else { "ip_cavlc.yuv" };
    ...
    let ok = Command::new("ffmpeg")
        .args([
            ...
            "-x264-params", &format!("{e}:ref=1:bframes=0:8x8dct=0:weightp=0:aud=0:no-deblock=1"),
            h264.to_str().unwrap(),
        ])
...
fn dump(annexb: &[u8], mb_cols: u32, mb_rows: u32, label: &str) {
    ...
    let p = units.iter().find(|u| u.nal_unit_type == NalUnitType::NonIdrSlice).expect("P");
    ...
    let header = tpt_kinetix_h264::slice::SliceHeader::parse_with_context(&p.rbsp, p.nal_unit_type, p.nal_ref_idc, &ctx).expect("hdr");
    ...
    for mb_y in 0..mb_rows {
        for mb_x in 0..mb_cols {
            ...
            let mvd = mb.motion.as_ref().map(|m| format!("{:?}", m.mvd_l0)).unwrap_or_else(|| "SKIP".to_string());
...
fn compare_cabac_vs_cavlc_mvd() {
    if !ffmpeg_available() { eprintln!("no ffmpeg"); return; }
    ...
    let mb_cols = 4u32; let mb_rows = 3u32;
