//! Phase G.5 PAFF B-field corpus validation (todo-h264.md).
//!
//! Decodes a PAFF B-field bitstream (generated by the JM reference encoder,
//! since x264 cannot produce PAFF) and compares against ffmpeg's reference
//! decode. Validates `decode_interlaced_b_field` and the field-coordinate
//! bi-predictive MC path.
...
fn paff_b_field_decode() {
    ...
    // Copy the pre-generated PAFF B-field bitstream into place.
    ...
    println!("ref_yuv: {} bytes", ref_yuv.len());

    let annexb = std::fs::read(&h264_path).unwrap();

    // Feed the whole Annex B stream as a single packet — the decoder handles
    // NAL splitting internally.
    ...
    if !frames.is_empty() && !ref_yuv.is_empty() {
        ...
        println!("ref has ~{ref_frames} frames of {frame_len} bytes");
        for (i, frame) in frames.iter().enumerate() {
            if frame.len() != frame_len {
                println!("frame#{i}: SKIP (wrong size {})", frame.len());
                continue;
            }
            if i < ref_frames {
                let off = i * frame_len;
                let (max_d, n_d, tot) = compare(frame, &ref_yuv[off..off + frame_len]);
                let luma_n = (frame.len() * 2) / 3;
                let (ld, ln, _) = compare(&frame[..luma_n], &ref_yuv[off..off + luma_n]);
                let (cd, cn, _) =
                    compare(&frame[luma_n..], &ref_yuv[off + luma_n..off + frame_len]);
                println!(
