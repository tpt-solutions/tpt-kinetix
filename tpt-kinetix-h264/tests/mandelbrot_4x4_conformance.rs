...
fn gen(dir: &std::path::Path) -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    let enc = Command::new("ffmpeg")
        .args([
            ...
            "yuv420p",
            "-x264-params",
            "cabac=0:8x8dct=0:ref=1:bframes=0:weightp=0:aud=0:no-deblock=1",
            h264.to_str()?,
        ])
        .output()
        .ok()?;
...
fn mandelbrot_4x4_cavlc_is_bitexact() {
    ...
    let (annexb, refyuv) = match gen(&dir) {
        Some(t) => t,
        None => {
            eprintln!("ffmpeg generation/decode failed; skipping");
            return;
        }
    };
    ...
    eprintln!(
        "mandelbrot 4×4 CAVLC vs ffmpeg: max_abs_diff={max_diff}, differing={num_diff}/{}",
        frame.data.len()
    );
    ...
    for i in 0..luma_len {
        ...
        if d != 0 {
            ...
            let mb_y = py / 16;
            eprintln!(
                "  First luma diff at pixel ({px},{py}) MB({mb_x},{mb_y}): ours={} ref={} diff={d}",
