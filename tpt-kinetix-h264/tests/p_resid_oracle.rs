...
fn gen() -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    let ok = Command::new("ffmpeg")
        .args([
            ...
            "-x264-params", "cabac=0:ref=1:bframes=0:8x8dct=0:weightp=0:aud=0:no-deblock:keyint=2:min-keyint=2",
            h264.to_str()?,
        ])
        .output()
        .map(|o| o.status.success())
...
impl DecodeTracer for Capture {
    fn on_cavlc_coeffs(
        ...
        if matches!(plane, TracePlane::Luma) {
            self.coeffs.lock().unwrap().insert((mb_x, mb_y, blk), *coeffs);
        }
    }
    fn on_mb_parsed(
        &mut self,
    ...
    for mb_y in 0..mb_rows {
        for mb_x in 0..mb_cols {
            for blk in 0u8..16 {
                ...
                let cells = parsed.mv_store.cells_of(mb_idx).unwrap_or([tpt_kinetix_h264::mv::MvCell::INTRA; 16]);
