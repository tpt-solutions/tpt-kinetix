...
fn ibp_p_slice_cabac_grid() {
    ...
    let ok = Command::new("ffmpeg")
        .args(["-hide_banner", "-loglevel", "error", "-y", "-f", "lavfi", "-i",
            "testsrc=size=64x64:rate=1:duration=3", "-c:v", "libx264", "-pix_fmt", "yuv420p",
            "-x264-params",
    ...
    let sps = units.iter().find(|u| u.nal_unit_type == NalUnitType::Sps)
        .and_then(|u| SeqParameterSet::parse(&u.rbsp).ok()).expect("sps");
    let pps = units.iter().find(|u| u.nal_unit_type == NalUnitType::Pps)
        .and_then(|u| PicParameterSet::parse(&u.rbsp, None).ok()).expect("pps");
    ...
    eprintln!("slice_qp={slice_qp} num_ref_l0={num_ref} cabac_init_idc={} t8={} mbaff={}",
    ...
    let parsed = parse_p_slice_cabac(
        r.remaining_bytes(), 4, 4, slice_qp, true, false,
        header.cabac_init_idc as usize, num_ref, pps.chroma_qp_index_offset,
        pps.transform_8x8_mode_flag, sps.direct_8x8_inference_flag,
        &mut tpt_kinetix_h264::trace::NoopTracer,
    ).expect("parse");

    // ---- B slice: parse + dump grid ----
    if let Some(b) = units.iter().filter(|u| u.nal_unit_type == NalUnitType::NonIdrSlice).nth(1) {
        ...
        eprintln!("\n=== B slice: qp={bqp} nl0={nl0} nl1={nl1} direct_spatial={:?} ===", bh.direct_spatial_mv_pred_flag);
        let mut br = BitReader::new(&b.rbsp);
        br.seek_to_bit(bh.data_bit_offset);
        br.byte_align();
        match tpt_kinetix_h264::slice_data::parse_b_slice_cabac(
            br.remaining_bytes(), 4, 4, bqp, true, false,
            bh.cabac_init_idc as usize, nl0, nl1, pps.chroma_qp_index_offset,
            pps.transform_8x8_mode_flag, sps.direct_8x8_inference_flag, None,
            &mut tpt_kinetix_h264::trace::NoopTracer,
        ) {
            Ok(bp) => {
                ...
                for gy in 0..4 {
                    for gx in 0..4 {
                        ...
                        let m = match &mb.motion {
                            ...
                            Some(mo) => format!("{:?}{} t8={} pd={:?} mvd0={:?} mvd1={:?}", mb.mb_type,
                                mo.sub_mb_type_b.map(|s| format!("{s:?}")).unwrap_or_default(),
                                mb.transform_size_8x8, mo.pred_dirs, mo.mvd_l0, mo.mvd_l1),
                        };
                        eprintln!("  gB({gx},{gy}) cbp={:02x} skip={} {m}", mb.cbp, mb.skip);
                        if let Some(cells) = bp.mv_store.cells_of(gy*4+gx) {
                            if !mb.skip {
                                let l0: Vec<[i32;2]> = [0usize,3,12,15].iter().map(|&c| cells[c].mv).collect();
    ...
    for d in 0..16usize {
        ...
        order.push_str(&format!("MB{d}(g{gi},{},{}){} ", gi % cols, gi / cols,
            if mb.skip { "S".into() } else { format!("{:?}", mb.mb_type) }));
    }
    eprintln!("decode-order: {order}");
    eprintln!("ffmpeg ground truth grid:  row0 S S S S / row1 > S S S / row2 > I > > / row3 >- >- >- >+");
    ...
    for gy in 0..4u32 {
        for gx in 0..4u32 {
            ...
            if let Some(cells) = parsed.mv_store.cells_of(gi) {
                let corners: Vec<[i32; 2]> = [0usize, 3, 12, 15].iter().map(|&c| cells[c].mv).collect();
    ...
    for gy in 0..4 {
        for gx in 0..4 {
            ...
            let m = match &mb.motion {
                None => format!("{:?}", mb.mb_type),
                Some(mo) => format!("{:?}{} t8={} mvd={:?}", mb.mb_type,
                    mo.sub_mb_type.map(|s| format!("{s:?}")).unwrap_or_default(),
                    mb.transform_size_8x8, mo.mvd_l0),
            };
            eprintln!("  g({gx},{gy}) cbp={:02x} field={} skip={} {m}", mb.cbp, mb.mb_field_flag, mb.skip);
        }
    }

    // ---- Full-decode + per-4x4-block SAD vs ffmpeg's P frame ----
    let ff = std::fs::read(dir.join("ffP.yuv")).ok().or_else(|| {
        let o = Command::new("ffmpeg")
            .args(["-hide_banner","-loglevel","error","-y","-i"]).arg(h264.to_str().unwrap())
            .args(["-vf","select=eq(pict_type\\,P)","-vsync","0","-f","rawvideo","-pix_fmt","yuv420p"])
            .arg(dir.join("ffP.yuv").to_str().unwrap()).output().unwrap();
    ...
    for i in 0..annexb.len().saturating_sub(3) {
        if annexb[i] == 0 && annexb[i+1] == 0 && annexb[i+2] == 1 { starts.push(i+3); }
    }
    let mut ours: Vec<Vec<u8>> = vec![];
    for (k,&s) in starts.iter().enumerate() {
        let e = starts.get(k+1).copied().unwrap_or(annexb.len());
        let mut data = vec![0u8,0,0,1]; data.extend_from_slice(&annexb[s..e]);
        let pkt = Packet { pts: Timestamp::new(k as i64,(1,30)), dts: Timestamp::new(k as i64,(1,30)),
            data, stream_index: 0, is_key_frame: true };
        if let Ok(Some(f)) = dec.decode(&pkt) { if f.data.len()==64*64*3/2 { ours.push(f.data); } }
    }
    ...
    let pf = ours.iter().min_by_key(|f| {
        f[..64*64].iter().zip(&ff[..64*64]).map(|(a,b)| (*a as i32-*b as i32).unsigned_abs() as u64).sum::<u64>()
    }).expect("no frames");
    let psad: u64 = pf[..64*64].iter().zip(&ff[..64*64]).map(|(a,b)| (*a as i32-*b as i32).unsigned_abs() as u64).sum();
    ...
    let ffb = {
        ...
        let o = Command::new("ffmpeg")
            .args(["-hide_banner","-loglevel","error","-y","-i"]).arg(h264.to_str().unwrap())
            .args(["-vf","select=eq(pict_type\\,B)","-vsync","0","-f","rawvideo","-pix_fmt","yuv420p"])
            .arg(p.to_str().unwrap()).output().unwrap();
        assert!(o.status.success());
        std::fs::read(&p).unwrap()
    };
    let bf = ours.iter().min_by_key(|f| {
        f[..64*64].iter().zip(&ffb[..64*64]).map(|(a,b)| (*a as i32-*b as i32).unsigned_abs() as u64).sum::<u64>()
    }).unwrap();
    let bsad: u64 = bf[..64*64].iter().zip(&ffb[..64*64]).map(|(a,b)| (*a as i32-*b as i32).unsigned_abs() as u64).sum();
