...
fn idr_bit_exact_check() {
    ...
    let ok = Command::new("ffmpeg")
        .args([
            ...
            "-g", "100", "-bf", "0", "-pix_fmt", "yuv420p",
            "-x264-params", "cabac=0:ref=1:bframes=0:8x8dct=0:weightp=0:aud=0:deblock=0",
            h264.to_str().unwrap(),
        ])
        .output().map(|o| o.status.success()).unwrap_or(false);
    assert!(ok, "ffmpeg gen");
    let ok = Command::new("ffmpeg")
        .args(["-hide_banner","-loglevel","error","-y","-i",h264.to_str().unwrap(),"-f","rawvideo","-pix_fmt","yuv420p",refyuv.to_str().unwrap()])
        .output().map(|o| o.status.success()).unwrap_or(false);
    assert!(ok, "ffmpeg decode");
    let annexb = std::fs::read(&h264).unwrap();
    let refy = std::fs::read(&refyuv).unwrap();
    let w = 64usize; let h = 48usize; let fl = w*h; let frame_len = fl*3/2;
    assert_eq!(refy.len(), frame_len);

    let mut dec = H264Decoder::new();
    let pkt = Packet { pts: Timestamp::new(0,(1,30)), dts: Timestamp::new(0,(1,30)), data: annexb, stream_index: 0, is_key_frame: true };
...
fn localize_pframe_diffs() {
    ...
    eprintln!("P-slice disable_deblocking_filter_idc={} slice_qp_delta={} first_mb={}", header.disable_deblocking_filter_idc, header.slice_qp_delta, header.first_mb_in_slice);
    let idr = units.iter().find(|u| u.nal_unit_type == NalUnitType::IdrSlice).expect("IDR");
    let idr_header = SliceHeader::parse_with_context(&idr.rbsp, idr.nal_unit_type, idr.nal_ref_idc, &ctx).expect("idr header");
    eprintln!("IDR disable_deblocking_filter_idc={} slice_qp_delta={}", idr_header.disable_deblocking_filter_idc, idr_header.slice_qp_delta);
    ...
    for mi in [7usize, 11usize] {
        ...
        let first = cells[0];
        eprintln!("MB({mb_x},{mb_y}) idx={mi} mv=[{},{}] ref={} skip={} cbp={:02x}", first.mv[0], first.mv[1], first.ref_idx, parsed.macroblocks[mi].skip, parsed.macroblocks[mi].cbp);
    }
    ...
    let mut max_self = 0i32; let mut n_self = 0usize;
    for yy in 0..16usize {
        for xx in 0..16usize {
            let px = 3*16 + xx; let py = 1*16 + yy;
            let a = i_full[py*w+px] as i32;
            let b = p_full[py*w+px] as i32;
            let d = (a-b).abs();
            if d>max_self {max_self=d;}
            if d!=0 {n_self+=1;}
        }
    }
    ...
    let mut d_vs_i = 0i32; let mut d_vs_p = 0i32;
    for yy in 0..16usize {
        for xx in 0..16usize {
            let px = 3*16 + xx; let py = 1*16 + yy;
            ...
            if di>d_vs_i {d_vs_i=di;}
            if dp>d_vs_p {d_vs_p=dp;}
        }
    }
    ...
    for mb_y in 0..mb_rows {
        for mb_x in 0..mb_cols {
            ...
            let tag = format!("MB({mb_x},{mb_y}) cbp={:02x} skip={} qp={} max_luma_diff={md}", mb.cbp, mb.skip, mb.qp);
            if mb.skip {
    ...
    eprintln!("=== CODED MBs ({}), max luma diff per MB ===", coded_diffs.len());
    ...
    let max_all = coded_diffs.iter().map(|(d, _)| *d).chain(skip_diffs.iter().map(|(d, _)| *d)).max().unwrap_or(0);
