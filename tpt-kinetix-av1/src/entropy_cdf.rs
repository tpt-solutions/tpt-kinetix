//! Default CDF tables for AV1 symbol decoding (AV1 spec §10, "Additional
//! tables").
//!
//! These are the `Default_*_Cdf` arrays a decoder copies into its
//! per-tile `Tile*Cdf` state at `init_symbol` time (spec §8.2.2) before
//! adapting them via [`crate::entropy::SymbolDecoder::read_symbol`].
//!
//! Covers, per AV1 Phase A scope (`todo.md`): `cbf`/`all_zero`
//! ([`DEFAULT_TXB_SKIP_CDF`]), `tx_type`
//! ([`DEFAULT_INTRA_TX_TYPE_SET1_CDF`] and friends), and coefficient-level
//! syntax ([`DEFAULT_EOB_PT_16_CDF`] through [`DEFAULT_COEFF_BR_CDF`]).
//!
//! Values were mechanically extracted (not hand-transcribed) from the
//! spec's C array-literal source and converted to Rust array-literal
//! syntax (`{` → `[`, `}` → `]`); element counts were verified against the
//! declared dimensions (e.g. `SIG_COEF_CONTEXTS = 42`) before inclusion.
//! Source:
//! <https://github.com/AOMediaCodec/av1-spec/blob/master/10.additional.tables.md>
//!
//! **Scope note**: these tables are indexed by the real spec context
//! dimensions (quantizer bucket, transform size, plane type, per-symbol
//! context), but nothing in this crate yet *computes* those context
//! indices (`SIG_COEF_CONTEXTS` neighbor derivation, `TXB_SKIP_CONTEXTS`
//! selection, etc.) — that context-selection logic is AV1 Phase B's job
//! (`todo.md`, "rewire coefficient decode onto the symbol decoder"). Phase
//! A only needs the raw default values to exist and be exercisable by the
//! symbol decoder engine.

#![allow(clippy::all)]


pub static DEFAULT_TXB_SKIP_CDF: [[[[u16;3];13];5];4] = [
  [
    [ [ 31849, 32768, 0 ],
      [ 5892, 32768, 0 ],
      [ 12112, 32768, 0 ],
...
pub static DEFAULT_INTRA_TX_TYPE_SET1_CDF: [[[u16;8];13];2] = [
    [
...
pub static DEFAULT_INTRA_TX_TYPE_SET2_CDF: [[[u16;6];13];3] = [
    [
...
pub static DEFAULT_INTER_TX_TYPE_SET1_CDF: [[u16;17];2] = [
...
pub static DEFAULT_INTER_TX_TYPE_SET2_CDF: [u16;13] = [
    770, 2421, 5225, 12907, 15819, 18927, 21561, 24089, 26595,
    28526, 30529, 32768, 0
];

pub static DEFAULT_INTER_TX_TYPE_SET3_CDF: [[u16;3];4] = [
    [ 16384, 32768, 0 ],
    [ 4167, 32768, 0 ],
    [ 1998, 32768, 0 ],
    [ 748, 32768, 0 ]
...
pub static DEFAULT_EOB_PT_16_CDF: [[[[u16;6];2];2];4] = [
...
pub static DEFAULT_EOB_PT_32_CDF: [[[[u16;7];2];2];4] = [
...
pub static DEFAULT_EOB_PT_64_CDF: [[[[u16;8];2];2];4] = [
...
pub static DEFAULT_EOB_PT_128_CDF: [[[[u16;9];2];2];4] = [
...
pub static DEFAULT_EOB_PT_256_CDF: [[[[u16;10];2];2];4] = [
...
pub static DEFAULT_EOB_PT_512_CDF: [[[u16;11];2];4] = [
...
pub static DEFAULT_EOB_PT_1024_CDF: [[[u16;12];2];4] = [
...
pub static DEFAULT_DC_SIGN_CDF: [[[[u16;3];3];2];4] = [
  [ [
      [ 128 * 125, 32768, 0 ],
      [ 128 * 102, 32768, 0 ],
      [ 128 * 147, 32768, 0 ],
...
pub static DEFAULT_COEFF_BASE_EOB_CDF: [[[[[u16;4];4];2];5];4] = [
  [ [ [ [ 17837, 29055, 32768, 0 ],
        [ 29600, 31446, 32768, 0 ],
        [ 30844, 31878, 32768, 0 ],
        [ 24926, 28948, 32768, 0 ] ],
...
pub static DEFAULT_COEFF_BR_CDF: [[[[[u16;5];21];2];5];4] = [
...
mod tests {
    ...
    /// Every leaf cdf array must end in `[..., 1 << 15, 0]`: the fixed top
    /// of the probability space followed by a freshly-initialized (zero)
    /// adaptation counter (spec §8.2.6 note; §8.2.2 init_symbol copy).
    fn assert_leaf_shape<'a, const N: usize>(name: &str, leaves: impl Iterator<Item = &'a [u16; N]>) {
        let mut count = 0;
        for (i, leaf) in leaves.enumerate() {
            assert_eq!(leaf[N - 2], 1 << 15, "{name}[{i}]: cdf[N-1] must be 32768");
            assert_eq!(leaf[N - 1], 0, "{name}[{i}]: adaptation counter must start at 0");
            count += 1;
        }
        assert!(count > 0, "{name}: no leaves visited");
    }

    #[test]
    fn all_default_cdf_leaves_are_well_formed() {
        assert_leaf_shape("DEFAULT_TXB_SKIP_CDF", DEFAULT_TXB_SKIP_CDF.iter().flatten().flatten());
        assert_leaf_shape(
            "DEFAULT_INTRA_TX_TYPE_SET1_CDF",
            DEFAULT_INTRA_TX_TYPE_SET1_CDF.iter().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_INTRA_TX_TYPE_SET2_CDF",
            DEFAULT_INTRA_TX_TYPE_SET2_CDF.iter().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_INTER_TX_TYPE_SET1_CDF",
            DEFAULT_INTER_TX_TYPE_SET1_CDF.iter(),
        );
        assert_leaf_shape(
            "DEFAULT_INTER_TX_TYPE_SET2_CDF",
            std::iter::once(&DEFAULT_INTER_TX_TYPE_SET2_CDF),
        );
        assert_leaf_shape(
            "DEFAULT_INTER_TX_TYPE_SET3_CDF",
            DEFAULT_INTER_TX_TYPE_SET3_CDF.iter(),
        );
        assert_leaf_shape("DEFAULT_EOB_PT_16_CDF", DEFAULT_EOB_PT_16_CDF.iter().flatten().flatten());
        assert_leaf_shape("DEFAULT_EOB_PT_32_CDF", DEFAULT_EOB_PT_32_CDF.iter().flatten().flatten());
        assert_leaf_shape("DEFAULT_EOB_PT_64_CDF", DEFAULT_EOB_PT_64_CDF.iter().flatten().flatten());
        assert_leaf_shape(
            "DEFAULT_EOB_PT_128_CDF",
            DEFAULT_EOB_PT_128_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_EOB_PT_256_CDF",
            DEFAULT_EOB_PT_256_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape("DEFAULT_EOB_PT_512_CDF", DEFAULT_EOB_PT_512_CDF.iter().flatten());
        assert_leaf_shape("DEFAULT_EOB_PT_1024_CDF", DEFAULT_EOB_PT_1024_CDF.iter().flatten());
        assert_leaf_shape("DEFAULT_DC_SIGN_CDF", DEFAULT_DC_SIGN_CDF.iter().flatten().flatten());
        assert_leaf_shape(
            "DEFAULT_COEFF_BASE_EOB_CDF",
            DEFAULT_COEFF_BASE_EOB_CDF.iter().flatten().flatten().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_COEFF_BASE_CDF",
            DEFAULT_COEFF_BASE_CDF.iter().flatten().flatten().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_COEFF_BR_CDF",
            DEFAULT_COEFF_BR_CDF.iter().flatten().flatten().flatten(),
        );
    }

    /// Sanity check that the tables can actually feed `read_symbol` and
    /// produce a plausible decode without panicking (shape/precondition
    /// smoke test, not a value-correctness test — that's covered by the
    /// `entropy` module's cross-validated golden vectors).
    #[test]
    fn coeff_base_cdf_decodes_without_panicking() {
        use crate::entropy::SymbolDecoder;

        let mut cdf = DEFAULT_COEFF_BASE_CDF[0][0][0][0];
        let mut dec = SymbolDecoder::new(&[0x3C, 0x91, 0xA0, 0x5F, 0x02, 0xE7]);
        for _ in 0..8 {
            let symbol = dec.read_symbol(&mut cdf);
            assert!(symbol < 4, "coeff_base has 4 possible values");
        }
    }
}
