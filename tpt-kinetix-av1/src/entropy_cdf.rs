...
pub static DEFAULT_EOB_EXTRA_CDF: [[[[[u16; 3]; 9]; 2]; 5]; 4] = [
    [
        [
            [
                [16961, 32768, 0],
...
mod tests {
    ...
    fn assert_leaf_shape<'a, const N: usize>(
        name: &str,
        leaves: impl Iterator<Item = &'a [u16; N]>,
    ) {
        let mut count = 0;
        for (i, leaf) in leaves.enumerate() {
            ...
            assert_eq!(
                leaf[N - 1],
                0,
                "{name}[{i}]: adaptation counter must start at 0"
            );
            count += 1;
        }
    ...
    fn all_default_cdf_leaves_are_well_formed() {
        assert_leaf_shape(
            ...
            DEFAULT_TXB_SKIP_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape(
        ...
        assert_leaf_shape(
            ...
            DEFAULT_EOB_PT_16_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_EOB_PT_32_CDF",
            DEFAULT_EOB_PT_32_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_EOB_PT_64_CDF",
            DEFAULT_EOB_PT_64_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape(
        ...
        assert_leaf_shape(
            ...
            DEFAULT_EOB_PT_512_CDF.iter().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_EOB_PT_1024_CDF",
            DEFAULT_EOB_PT_1024_CDF.iter().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_EOB_EXTRA_CDF",
            DEFAULT_EOB_EXTRA_CDF.iter().flatten().flatten().flatten(),
        );
        assert_leaf_shape(
            "DEFAULT_DC_SIGN_CDF",
            DEFAULT_DC_SIGN_CDF.iter().flatten().flatten(),
        );
        assert_leaf_shape(
