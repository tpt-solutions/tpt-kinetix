...
| VP9 decode | 🟡 Not pixel-exact | ⚖️ Royalty-free. Profile-0 (8-bit 4:2:0) pipeline implemented end-to-end (headers, bool decoder, tiles/partitions/modes/MVs, coefficients, transforms, intra/inter prediction, loop filter, frame-context adaptation, superframes); tables extracted and `verify-tables`-checked vs pinned ffmpeg; solid-color clips (16-96px, lossless + lossy, 1-2 SB rows) decode pixel-exact; content-rich clips still desync in the coefficient walk (`tpt-kinetix-vp9`, see `todo-vp9.md`) |
