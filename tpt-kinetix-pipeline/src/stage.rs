...
pub struct EncodeStage {
    config: tpt_kinetix_core::encode::EncodeConfig,
}

impl EncodeStage {
    /// Create an encode stage from a codec-agnostic [`tpt_kinetix_core::encode::EncodeConfig`].
    pub fn new(config: tpt_kinetix_core::encode::EncodeConfig) -> Self {
        Self { config }
    }
}
...
impl Stage for EncodeStage {
    ...
    fn spawn(
        ...
        std::thread::spawn(move || {
            let mut encoder: Option<tpt_kinetix_av1::Av1Encoder> = None;
                ...
                match msg {
                    PipelineMessage::Frame(frame) => {
                        ...
                        if encoder.is_none() {
                            ...
                            match tpt_kinetix_av1::Av1Encoder::from_encode_config(cfg) {
                                Ok(e) => encoder = Some(e),
                                Err(e) => {
                                    output.send(PipelineMessage::Error(e.to_string())).ok();
                                    continue;
