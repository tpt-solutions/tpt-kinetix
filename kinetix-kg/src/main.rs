//! `kinetix-kg` — knowledge-graph ingestion and Rust codegen CLI.
//!
//! # Subcommands
//!
//! | Command | Description |
...
enum Commands {
    ...
    Analyze {
        /// Path to a graph JSON file produced by `kinetix-kg graph`.
        graph_json: PathBuf,
    },
    ...
    Codegen {
        ...
        #[arg(long, default_value = "kinetix-codec")]
        crate_name: String,
...
fn build_combined_graph(ast: &CAst) -> KnowledgeGraph {
    ...
    for node in mb_graph.nodes {
        graph.nodes.push(kinetix_kg::graph::Node {
            id: node.id + offset,
            kind: node.kind,
        });
    }
    for edge in mb_graph.edges {
        graph.edges.push(kinetix_kg::graph::Edge {
