//! End-to-end integration test for the `kinetix-kg` Phase 1 pipeline.
//!
...
use kinetix_kg::analysis::{find_independent_sets, mark_parallel_regions};
use kinetix_kg::codegen::{generate, CodegenOptions};
use kinetix_kg::extraction::{extract_bitstream_parsing_tree, extract_macroblock_state_machine};
use kinetix_kg::graph::{Edge, KnowledgeGraph, Node, NodeKind};
use kinetix_kg::ingestion::CAst;
...
fn discovers_call_edges() {
    ...
    let out_calls = graph
        .edges
        .iter()
        .filter(|e| e.from == nal && matches!(e.kind, kinetix_kg::graph::EdgeKind::Calls))
        .count();
    assert!(
        out_calls >= 1,
